...
public class XmlStatementReaderTest {
	...
	public void testInputIsNull() throws ProcessorException {
		...
		assertNotNull(xmlStatementReader);
		xmlStatementReader.readXmlFile(null);
	}

	@Test
	public void testRecordsAvailable() throws Exception {
		Path path = Paths.get(ClassLoader.getSystemResource(file).toURI());
		List<StatementDTO> statements = new XmlStatementReader().readXmlFile(path);
